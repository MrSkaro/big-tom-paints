    ...
    return ReactDom.createPortal(
        <div className="fixed top-0 left-0 w-screen h-screen z-50">
            <div onClick={closeModal} className="bg-transparent absolute inset-0"></div>
                <div className="flex flex-col bg-white gap-4 absolute right-0 top-0 h-screen w-screen sm:w-96 max-w-screen shadow-lg">
                        ...
                        {cartItems.length === 0 ? (
                            ...
                            <>
                                {cartItems.map((cartItem, itemIndex) => {
                                    return (
                                        <div key={itemIndex} className="flex flex-col gap-2 border-l border-solid border-slate-700 px-2">
                    ...
                    <div onClick={checkout} className='border border-solid border-slate-700 text-xl m-4 p-6 uppercase grid place-items-center hover:opacity-50 cursor-pointer'>Checkout</div>
                </div>
        </div>,
